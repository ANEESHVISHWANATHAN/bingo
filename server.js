const express = require('express'); const path = require('path'); const WebSocket = require('ws'); const http = require('http');

const app = express(); const server = http.createServer(app); const wss = new WebSocket.Server({ server });

app.use(express.static(__dirname));

app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'index.html'))); app.get('/profile', (req, res) => res.sendFile(path.join(__dirname, 'profile.html'))); app.get('/prelobby', (req, res) => res.sendFile(path.join(__dirname, 'prelobby.html'))); app.get('/lobby', (req, res) => res.sendFile(path.join(__dirname, 'lobby.html'))); app.get('/tambola', (req, res) => res.sendFile(path.join(__dirname, 'tambola.html'))); app.get('/tambolastandings', (req, res) => res.sendFile(path.join(__dirname, 'tambolastandings.html')));

const lobbies = {}; const pendingDisconnects = new Map();

function generateUniqueRoomId() { let id; do id = Math.random().toString(36).substring(2, 7); while (lobbies[id]); return id; }

function generateUniqueWsCode() { let code; do { code = Math.random().toString(36).substring(2, 10); } while (Object.values(lobbies).some(lobby => lobby.players.some(p => p.wscode === code))); return code; }

function shuffleArray(arr) { for (let i = arr.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [arr[i], arr[j]] = [arr[j], arr[i]]; } }

function generateTicket() { let numbers = []; while (numbers.length < 15) { const num = Math.floor(Math.random() * 100); if (!numbers.includes(num)) numbers.push(num); } return numbers; }

function hasMoreThan3Common(arr1, arr2) { return arr1.filter(n => arr2.includes(n)).length > 3; }

wss.on('connection', (ws) => { console.log('New WebSocket connection established.');

ws.on('message', (message) => { let msg; try { msg = JSON.parse(message); } catch { console.log('Received invalid JSON message'); return; }
...
if (msg.type === 'createlobby') {
  ...
  if (player.wsindex === 2 && game === 'monopoly') {
    console.log(`Monopoly page entered: ${player.username}`);
    // Future monopoly game logic here
  }
  ...
  if (arrid === 0) {
    for (const p of lobby.players) {
      if (p.ws.readyState === WebSocket.OPEN) {
        p.ws.send(JSON.stringify({
          type: 'varfirstdone', var: stage, username, icon, plyrid, points
        }));
      }
    }
  } else {
    for (const p of lobby.players) {
      if (p.ws.readyState === WebSocket.OPEN) {
        p.ws.send(JSON.stringify({
          type: 'varothersdone', var: stage, username, icon, plyrid, points, arrid
        }));
      }
    }
  }
...
ws.on('close', () => { for (const [roomid, lobby] of Object.entries(lobbies)) { for (const player of lobby.players) { if (player.ws === ws) { const plyrid = player.plyrid; player._disconnectTimeout = setTimeout(() => { lobby.players.splice(plyrid, 1); for (let i = plyrid; i < lobby.players.length; i++) { lobby.players[i].plyrid--; } for (const p of lobby.players) { if (p.ws.readyState === WebSocket.OPEN) { p.ws.send(JSON.stringify({ type: 'someuserleft', plyrid })); } } if (lobby.players.length === 0) { delete lobbies[roomid]; } }, 10000); break; } } } }); });
